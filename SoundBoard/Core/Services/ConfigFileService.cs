...
public class ConfigFileService(string workingPath)
{
    ...
    public void Load()
    {
        ...
        var fileData = System.IO.File.ReadAllText(workingPath);
        if (fileData.Length == 0)
            return;

        var soundItems = JsonConvert.DeserializeObject<SoundItemConfig[]>(fileData);
    ...
    public void Save()
    {
        ...
        var data = JsonConvert.SerializeObject(soundItems);
        System.IO.File.WriteAllText(workingPath, data);
    }
}